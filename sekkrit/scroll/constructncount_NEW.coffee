...
FunctionNode = ImmutableRecordExtending AbstractNode,
  ...
  ctor: (func, childNodes) ->
    @super {func, childNodes}

  evaluate: (evaluationContext) ->
    evaluatedChildNodes = @childNodes.invoke('evaluate', evaluationContext).toJS()
    @func(evaluatedChildNodes)

  applyTypingContext: (typingContext, path) ->
    typedChildNodes = @childNodes.map (childNode, key) ->
      childNode.applyTypingContext(typingContext, path.push('nodes', key))

    addTyping(
      @set('childNodes', typedChildNodes)
      typingContext
      @getTypeFromChildTypes(typedChildNodes.pluck('type'))
      path
    )

  getTypeFromChildTypes: (childTypes) ->
    throw 'Not implemented!'

  getView: (childTypes) ->
    throw 'Not implemented!'

  viewForChild: (childNameOrNode, nodeViewContext) ->
    node = if _.isString(childNameOrNode)
      @childNodes.get(childNameOrNode)
    else
      childNameOrNode
...
TupleNode = ImmutableRecordExtending FunctionNode,
  ...
  getTypeFromChildTypes: (childTypes) ->
    new TupleType(childTypes)

  getView: (nodeViewContext) ->
    [
      '['
      @childNodes.mapToJS (childNode, i) =>
        [
          @viewForChild(childNode, nodeViewContext)
  ...
  toString: ->
    'TUPLE(' + @childNodes.invoke('toString').join(', ') + ')'

SingletonNode = ImmutableRecordExtending FunctionNode,
  ctor: (childNode) ->
    childNodes = Immutable.Map({childNode})
    func = ({childNode: childValue}) -> [childValue]
    @super {func, childNodes}

  getTypeFromChildTypes: (childTypes) ->
    new SetType(childTypes.get('childNode'))
  ...
  toString: ->
    "SINGLETON(#{@childNodes.get('childNode').toString()})"
...
PickNode = ImmutableRecordExtending SingletonNode,
  ...
  toString: ->
    "PICK(#{@childNodes.get('childNode').toString()})"

MinusNode = ImmutableRecordExtending FunctionNode,
  ctor: (node1, node2) ->
    childNodes = Immutable.Map({node1, node2})
    func = ({node1: value1, node2: value2}) -> _.difference(value1, value2)
    @super {childNodes, func}

  getTypeFromChildTypes: (childTypes) ->
    childTypes.get('node1')

  getName: -> 'Minus'

  getView: (nodeViewContext) ->
    [
      @viewForChild('node1', nodeViewContext)
      " - "
      @viewForChild('node2', nodeViewContext)
    ]

  toString: ->
    "MINUS(#{@childNodes.get('node1').toString()}, #{@childNodes.get('node2').toString()})"

RangeNode = ImmutableRecordExtending FunctionNode,
  ctor: (startNode, endNode) ->
    nodes = Immutable.Map({startNode, endNode})
    func = (({startNode: startValue, endNode: endValue}) -> [startValue..endValue])
    @super {func, nodes}
  ...
  getView: (nodeViewContext) ->
    [
      'Count from '
      @viewForChild('startNode', nodeViewContext)
      ' to '
      @viewForChild('endNode', nodeViewContext)
    ]

  toString: ->
    "RANGE(#{@childNodes.get('startNode').toString()}, #{@childNodes.get('endNode').toString()})"
