...
require ['pins', 'boxes', 'cpmc'], (pins, boxes, cpmc) ->
  ...
  do ->
    ...
    board = svg.append("g").classed("board", true).boxify("board").pin
      tl:     [0, 0]
      height: 120
    mBrace = svg.append("g").boxify("brace").call(brace, dir: "left")
    .style("stroke", "black")
    .pin
      tl: boxes.translate(board.pin("tr"), [10, 0])
      bl: boxes.translate(board.pin("br"), [10, 0])
      width: 10
    mText = svg.append("text").boxify("text").pin
      tl: boxes.translate(mBrace.pin("mr"), [10, 0])
    .text("m").style("font-style", "italic")
    .style("text-anchor", "middle").style("dominant-baseline", "central")
    nBrace = svg.append("g").boxify("brace").call(brace, dir: "up")
    .style("stroke", "black")
    .pin
      tl: boxes.translate(board.pin("bl"), [0, 10])
      tr: boxes.translate(board.pin("br"), [0, 10])
      height: 10
    nText = svg.append("text").boxify("text").pin
      tl: boxes.translate(nBrace.pin("bm"), [0, 10])
    .text("n").style("font-style", "italic")
    .style("text-anchor", "middle").style("dominant-baseline", "central")
   
    svg.boxify("svg").pin({
      tl: board.pin("tl")
      right: boxes.translate(mBrace.pin("right"), 20)
  ...
  lilBoard = (name, [m,n]=[3,4]) ->
    svg = d3.select("##{name} svg").boxify()

    board = svg.append("g").classed("board", true).boxify("board").pin
      tl: [0,0]
      height: 120
      all: svg.pin("all")
    boardModel = new BoardModel(m,n)
    boardView = new BoardView(board, boardModel)
    boardView.domMargin = 5
    return boardView


  do ->
    boardView = lilBoard("diag1")
    boardModel = boardView.model
    boardModel.dominoes = [new Domino([[1,1],[2,1]]),
                           new Domino([[1,2],[1,3]])]
    boardModel.events.change()

  do ->
    boardView = lilBoard("diag2")
    boardModel = boardView.model
    boardModel.dominoes = boardModel.allTilings()[0]
    boardModel.events.change()

  do ->
    boardView = lilBoard("diag3")
    boardModel = boardView.model
    boardModel.dominoes = boardModel.allTilings()[3]
    boardModel.events.change()
    #board.selectAll("rect.domino").style("fill-opacity", 0.7)

  do ->
    boardView = lilBoard("diag4", [2,2])
    boardView.domMargin = 10
    boardModel = boardView.model
    boardModel.dominoes = [new Domino([[0,0],[0,1]])]
    boardModel.events.change()

  do ->
    boardView = lilBoard("diag5", [2,2])
    boardView.domMargin = 10
    boardModel = boardView.model
    boardModel.dominoes = [new Domino([[0,0],[0,1]]), new Domino([[0,0],[1,0]]),
                           new Domino([[1,0],[1,1]])]
    boardModel.events.change()

  do ->
    boardView = lilBoard("diag6", [2,3])
    boardView.domMargin = 10
    console.log boardView.sel.selectAll(".square .background")
    .filter((d) -> d[1] == 2).style("fill", "none")
    boardModel = boardView.model
    boardModel.dominoes = [new Domino([[0,0],[1,0]]), new Domino([[0,1],[0,2]]),
                           new Domino([[1,1],[1,2]])]
    boardModel.events.change()

  do ->
    boardModel = new BoardModel(3,4)
    tilings = boardModel.allTilings()
    tilingsRows = ([i...Math.min(i+4,tilings.length)] for i in [0...tilings.length] by 4)

    table = d3.select("#diag7")
    table.selectAll("tr").data(tilingsRows, String)
    .enter().append("tr").selectAll("td").data((d) -> d)
    .enter().append("td").style("padding", "7px").each (d) ->
      svg = d3.select(@).append("svg").boxify()
      ...
      boardView = new BoardView(board, boardModel)
      boardView.domMargin = 5
      boardModel.dominoes = boardModel.allTilings()[d]
      boardModel.events.change()
